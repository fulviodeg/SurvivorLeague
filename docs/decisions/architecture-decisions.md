...
6. **Atomicità (D6).** Il decremento di `jollies_remaining` avviene nella STESSA transazione dell'inserimento pick: `db.transaction` in `registerPick` (insert + `UPDATE profile SET jollies_remaining = jollies_remaining - 1` solo se `input.jolly === true`); dentro il `BEGIN/COMMIT` esistente in `autoJoinFromPick`. Violazione UNIQUE → `pick_already_exists` (invariato). Mai un consumo senza pick e mai un pick con jolly senza consumo.
