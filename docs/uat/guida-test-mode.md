...
**Nota email v3 Parte B (parser deterministico, ADR-014).** Con `AI_EMAIL_PARSER=false` (default) l'intento NON è classificato dall'LLM ma da un parser deterministico che riconosce SOLO formule univoche nel subject o nel corpo: `ISCRIZIONE [NOME]` (iscrizione, nome dopo la keyword), `DISISCRIZIONE` (disiscrizione), **`PARTECIPO` (partecipazione al torneo, ADR-019)** e `<TEAM> <ESITO>` (pick: squadra — nome canonico o alias — + esito `win`/`draw`/`lose` o sinonimi italiani). Qualunque altra cosa → chiarimento (`other`). Le formule libere ("voglio iscrivermi", "mi iscrivo") NON sono riconosciute. Con `AI_EMAIL_PARSER=true` si usa l'LLM con fallback per-messaggio sul deterministico (su `LLMError`) **e — da 2026-08-31, bug UAT 2026-08-30 — anche sugli esiti dubbiosi (`other`/`pick:null`): se il deterministico riconosce un pick valido (es. un nome abbreviato come "Parma"), vince lui con un warn `llm_false_negative` — un nome abbreviato valido non deve MAI finire in chiarimento**. Il filtro del classificatore LLM risolve inoltre gli alias verso il nome canonico (modulo condiviso `team-terms`). Con `AI_EMAIL_GENERATOR=false` e `AI_EMAIL_PARSER=false` la `LLM_API_KEY` non è richiesta (run senza IA).
...
**Nota Jolly (ADR-018).** La feature Jolly (`JOLLIES_PER_PLAYER`, int ≥ 0, default `1`, in `.env.uat` è già `1`) è una **funzionalità di sistema, non del test mode**: in UAT basta lasciare `JOLLIES_PER_PLAYER=1` per dimostrarla (o `0` per disattivarla — il sistema si comporta come oggi e la keyword "jolly" è ignorata). Con i jolly attivi e `WIN_ONLY=true`, il parser deterministico riconosce la keyword **`<TEAM> Jolly`** ovunque nel testo (prima o dopo la squadra, maiuscole/accenti-insensibili, es. "Napoli Jolly"): il pick viene registrato con il jolly (`pick.jolly_used=1`, contatore `jollies_remaining` decrementato) e la conferma è **"PICK REGISTRATO CON JOLLY → NAPOLI"**. Effetto in `win_only`: se la squadra scelta **pareggia**, il jolly **salva dall'eliminazione** (mail "🎯 Il tuo jolly ti ha salvato: NAPOLI ha pareggiato."); se **perde**, il jolly NON salva (eliminazione, "🎯 Il jolly non salva dalla sconfitta."); se **vince**, resta in gara ("🎯 Jolly usato"). Il jolly è **bruciato alla dichiarazione**: quando `jollies_remaining` arriva a `0`, un pick con "jolly" è rifiutato con "non hai più jolly disponibili". Anche `JOLLIES_PER_PLAYER` è **fissata a `tournament:start`** e coperta dalla stessa guardia fatal: impostarla PRIMA dell'avvio e non modificarla a torneo aperto.
...
| **TTnTCm** | Il token compatto che identifica un turno: `TT` numero del turno di torneo, `TC` numero della giornata di campionato (es. `TT2TC7` = secondo turno di gioco, agganciato alla giornata 7). Compare nelle righe di `tournament:history` e nei log; nell'oggetto delle email NON compare alcun turno (solo l'etichetta del tipo, es. "Pick Registrato"), la coppia estesa "Round del torneo N · Turno di Campionato M" sta nel corpo (email v4, ADR-015). In TEST MODE l'oggetto è preceduto da `🚧⚠️TEST MODE⚠️🚧 - ` (mai confondere una mail di test con una reale). |
