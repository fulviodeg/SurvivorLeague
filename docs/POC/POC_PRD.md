...
|---|---|---|
| Iscrizione via email | §1, §3.1 | Account **piattaforma** persistente (email, `registerID` stabile); la **partecipazione** al torneo (profilo) è **opt-in** (ADR-019): nasce per **auto-join a `tournament:start`** (account `active` con `tournament_auto_join = ON`) o per **dichiarazione esplicita** (`PARTECIPO` / `tournament:join`) nella finestra del TT 1 |
...
| Partecipazione opt-in (registration ≠ join) | — (2026-08-30, ADR-019) | Due flag per-account nel DB piattaforma (`tournament_auto_join`, `receive_tournament_start_notification`, default ON, gestiti SOLO via CLI `platform:register`/`platform:preferences`); profilo creato da `tournament:start` (auto-join) o da dichiarazione `PARTECIPO`/`tournament:join`; **nessun auto-join al primo pick** |
...
| **Partecipante** | Un iscritto che ha un **profilo** nel torneo (ADR-019): nato per **auto-join a `tournament:start`** o per **dichiarazione esplicita** (`PARTECIPO`/`tournament:join`). Solo i partecipanti ricevono le email di round |
| **Registration (iscrizione)** | La relazione giocatore↔**piattaforma** (ADR-009/019): account persistente che **sopravvive** alla creazione e distruzione dei tornei. Intenti `subscribe`/`unsubscribe`, comandi `platform:*` |
| **Join (partecipazione)** | La partecipazione al **singolo torneo** (ADR-019): il `profile` nasce e muore col torneo. Intento `join` (formula `PARTECIPO`), comando `tournament:join` |
| **Flag di partecipazione** | Le due preferenze per-account nel DB piattaforma (ADR-019): `tournament_auto_join` (1 = auto-join a `tournament:start`) e `receive_tournament_start_notification` (1 = riceve la mail `tournament_open`). Default ON; gestite SOLO via CLI (`platform:register`/`platform:preferences`) |
...
| **Token `TTnTCm`** | Coppia numerica compatta "TT n, TC m" (es. `TT2TC7`) usata in CLI e log; nelle email il corpo porta la coppia in forma estesa "Round N · Turno di campionato M", il soggetto NON porta turni (solo l'etichetta del tipo, es. "Pick Registrato"; in TEST MODE è preceduto da "🚧⚠️TEST MODE⚠️🚧 - ") (RF-25, ADR-013); nei log sono campi strutturati `{tt, tc}` (RF-25) |
...
| **Profilo** | La **partecipazione al torneo** di un iscritto (ADR-019): nasce per **auto-join a `tournament:start`** o per **dichiarazione esplicita** (`PARTECIPO`/`tournament:join`) nella finestra del TT 1, e muore per eliminazione. Nella POC: un profilo per giocatore |
...
- **RF-25** — La mappatura TT↔TC è presente in ogni comunicazione, log strutturato e output CLI (coppia `{tt, tc}`), iniettata deterministicamente nei template, mai generata dall'LLM. **Emendamento email v3 (ADR-013):** nel SOGGETTO compare la sola etichetta del tipo (`⚽🏆SURVIVOR LEAGUE🏆⚽ - {etichetta}`); la coppia "Round N · Turno di campionato M" resta nel CORPO. **Emendamento 2026-08-31 (richiesta PO):** rimosso dal soggetto anche il turno di campionato (il numero è già nel corpo) e, in TEST MODE (`TEST_MODE=true`), il brand è preceduto da `🚧⚠️TEST MODE⚠️🚧 - `.
...
**Partecipazione al torneo: opt-in (RF-P5, ADR-019).** Il profilo (partecipazione) nasce **solo** in uno di questi due modi:

1. **Auto-join a `tournament:start`.** Gli account piattaforma `active` con `tournament_auto_join = ON` vengono **auto-iscritti** al torneo all'avvio (snapshot unico: un account diventato `active` dopo `tournament:start` NON è auto-joinato — late registrant, deve dichiarare);
2. **Dichiarazione esplicita (join).** Via email con la formula **`PARTECIPO`** o via CLI con `tournament:join --email <email>`, nella finestra del TT 1 (round 1 in stato `pending` o `open`, anche prima di `round:open`). Riuscita → `tournament_join_confirmed`; account già in gara → `tournament_already_joined` (idempotenza: flag ON + `PARTECIPO` = "già in gara").

Chi si iscrive alla piattaforma durante un torneo aperto **non** è quindi automaticamente partecipante: entra solo per auto-join a start o dichiarando. **Un pick NON crea mai più un profilo** (rimosso l'auto-join al primo pick): un iscritto `active` senza profilo che invia un pick nel TT 1 riceve `tournament_join_rejected` con reason `not_in_tournament` (testo "per partecipare invia PARTECIPO"); **dopo la chiusura del TT 1** la lista partecipanti è **blindata**: la dichiarazione è rifiutata (il torneo è iniziato) e l'unico ingresso tardivo è l'override CLI `tournament:join --reason <motivo>` (ADR-008 §6, profilo creato con pool intatto). La **disiscrizione a torneo in corso** non tocca il profilo (storico intatto): ferma solo comunicazioni e pick; il profilo muore naturalmente alla prossima chiusura round (`missing_pick`, senza email al disiscritto). Se il giocatore si **re-iscrive prima della prossima deadline**, riprende a giocare con lo stesso `registerID` e lo stesso profilo.

**Flag di partecipazione (gestiti SOLO via CLI).** Due preferenze per-account nel DB piattaforma (canale-agnostiche, default ON): `tournament_auto_join` (1 = auto-join a `tournament:start`) e `receive_tournament_start_notification` (1 = riceve la mail `tournament_open`). Si impostano alla registrazione (`platform:register --auto-join --receive-notifications`, default `true`) o dopo con `platform:preferences --email [--auto-join on|off] [--receive-notifications on|off]`. Un cambio flag impatta **solo i tornei successivi** (snapshot a `tournament:start`); **nessuna email** serve a togglarli. `platform:list` mostra i due flag.
...
- Il gate di eligibilità (`checkEligibility(ExternalIdentity)`, ADR-008) resta: l'implementazione POC è "account piattaforma **attivo**" (Fase 1: attivo + quota pagata). Il gate del pick = piattaforma attiva + profilo (ADR-019: nessun auto-join al pick)
...
- **RF-P1** — Iscrizione piattaforma via email (intento LLM o deterministico, ADR-014): crea/riattiva l'account con `registerID` stabile; conferma via email (`platform_registered`). Già iscritto → risposta "già iscritto" con il tipo email dedicato `platform_already_registered` (ADR-010). **Emendamento ADR-011:** il classificatore deduce dalla mail di registrazione anche il **nome** del giocatore (`{intent, pick, name?}`), salvato su `platform_account.name` alla prima creazione; nome assente → il sistema usa l'indirizzo email al posto del nome (correzione `registered_user_name`). **Emendamento ADR-014:** l'istruzione d'iscrizione ovunque è la formula `ISCRIZIONE [NOME]` (es. mail di chiarimento), che sostituisce la vecchia "dici voglio iscrivermi".
...
- **RF-P5** — Partecipazione opt-in al torneo (ADR-019): il profilo nasce per **auto-join a `tournament:start`** (account `active` con `tournament_auto_join = ON`) o per **dichiarazione esplicita** (email `PARTECIPO` o CLI `tournament:join`) nella finestra del TT 1 (round 1 `pending` o `open`). Dopo la chiusura del TT 1 → rifiuto con risposta; unico ingresso tardivo = override CLI `tournament:join --reason` (obbligatorio). Un pick **non crea mai più un profilo** (rimosso l'auto-join al primo pick). Disiscritto a torneo in corso → profilo conservato, comunicazioni e pick fermati; re-iscrizione prima della prossima deadline → si riprende con lo stesso `registerID` e profilo.
- **RF-P6** — Notifiche: apertura torneo (`tournament_open`) ai soli account `active` con `receive_tournament_start_notification = ON`; apertura round (`pick_instructions`) ai soli partecipanti attivi (profili `eliminated = 0`); chiusura round (riepilogo `round_closed_survived`) ai soli sopravvissuti; gli eliminati ricevono solo le notifiche puntuali (`pick_missing_elimination`, `round_result_wrong`). **Carve-out (ADR-010):** le conferme del flusso di iscrizione/disiscrizione (RF-P1/P2 — `platform_unsubscribe_confirm`, `platform_unsubscribed`, risposte subscribe) partono **sempre**, anche verso account non `active`, perché sono il flusso di conferma stesso; **ADR-019:** il flusso di **join** (`tournament_join_confirmed`/`tournament_already_joined`/`tournament_join_rejected`) è il flusso di partecipazione e parte sempre verso l'account `active` che dichiara; il filtro `active` si applica a tutte le altre email. **Emendamento ADR-011/ADR-013:** cambiano SOLO i testi (stile unico "energetic", renderer deterministico di canale, soggetti con il solo turno di campionato e neutri per gli esiti, corpo plain-text senza riquadri ASCII, mai elenchi nominativi di partecipanti, chiusura fissa dell'eliminato); destinatari, filtri e matrice restano INVARIATI (salvo ADR-019).
...
3. Il sistema invia l'email di **apertura torneo** (`tournament_open`, una sola volta) ai **soli account attivi con `receive_tournament_start_notification = ON`** (RF-P6/ADR-019: sostituisce l'invito a una lista di contatti) e, a ogni apertura di TT, un'email a **ogni partecipante attivo** (profilo `eliminated = 0` con account piattaforma `active`) contenente:
...
- **RF-06** — Il sistema invia l'email pick di apertura TT **ai soli partecipanti attivi** (`eliminated = 0` con account piattaforma `active`), con coppia TT/TC, squadre disponibili, deadline e istruzioni (RF-P6). **ADR-019:** nessuna eccezione TT1 per gli iscritti senza profilo (la nascita dei profili non è più legata al pick) — chi non è partecipante dichiara con `PARTECIPO`/`tournament:join`.
...
3. Il sistema valida il pick:
   - L'account piattaforma del mittente è `active` e il mittente ha un profilo in gara? (ADR-019: **nessun auto-join al pick** — un iscritto senza profilo riceve `tournament_join_rejected` reason `not_in_tournament` nel TT1 o `rejected_tt2` dal TT2. Un mittente **non iscritto** riceve solo un log interno, RF-P4.)
...
- Un pick da un iscritto **senza profilo**: **NON crea mai più un profilo** (ADR-019, rimosso l'auto-join al primo pick). Nel TT 1 con pick riconoscibile → `tournament_join_rejected` reason `not_in_tournament` (testo "per partecipare invia PARTECIPO"); dal TT 2 → rifiuto "il torneo è iniziato" (`rejected_tt2`)
...
- **RF-10** — La validazione verifica: account piattaforma `active`, profilo esistente in gara (ADR-019: **nessun auto-join al pick**), squadra in partita nel TC, squadra non bruciata nel girone, esito valido, entro l'istante di accettazione.
...
5. Alla **transizione `closed → scored`** (e solo lì, una sola volta: guardia `round_state.summary_sent`, RF-P6) il sistema invia il **riepilogo di chiusura round** `round_closed_survived` **ai soli sopravvissuti** (`eliminated = 0` con account piattaforma `active`). Gli eliminati ricevono **solo** le notifiche puntuali (`pick_missing_elimination` alla chiusura, `round_result_wrong` alla contabilizzazione): **nessun** riepilogo di chiusura. L'eliminazione a posteriori da Freeze produce **solo** `round_result_wrong` (coerente con PRD §5.4), nessun riepilogo. Gli eliminati dei round precedenti non ricevono più email di round; chi è iscritto ma non partecipa riceve l'apertura torneo solo se ha `receive_tournament_start_notification = ON` (ADR-019). **Ogni email in uscita è filtrata sullo stato dell'account piattaforma al momento dell'invio**: `unsubscribed`/`pending_unsubscribe` non ricevono alcuna email — **salvo le conferme del flusso di iscrizione/disiscrizione** (RF-P1/P2, ADR-010) e del flusso di **join** (ADR-019), che partono sempre perché sono il flusso di conferma stesso.
...
- Data un'iscrizione da un account `pending_unsubscribe` o `unsubscribed`, il sistema lo riattiva ad `active` con lo **stesso `registerID`** (RF-P3; i flag di partecipazione NON sono toccati dalla riattivazione).
- Dato un account `active` con `tournament_auto_join = ON`, a `tournament:start` il sistema **auto-join** il profilo (snapshot unico; idempotente se già presente).
- Data una **dichiarazione** `PARTECIPO`/`tournament:join` di un iscritto `active` senza profilo nella finestra del TT 1, il sistema crea il profilo e risponde `tournament_join_confirmed` (RF-P5).
- Dato un account **già in gara** che dichiara ancora, il sistema risponde `tournament_already_joined` (idempotenza).
- Data una dichiarazione **dopo la chiusura del TT 1**, il sistema la rifiuta (il torneo è iniziato); l'ingresso tardivo è **solo** via CLI con `tournament:join --reason <motivo>` (override auditato).
- Dato un pick nel TT 1 da un mittente **non iscritto**, il sistema **non** auto-iscrive: log interno, nessuna risposta (RF-P4).
- Dato un pick da un iscritto senza profilo, il sistema **non crea il profilo**: nel TT1 risponde `tournament_join_rejected` reason `not_in_tournament` (testo "per partecipare invia PARTECIPO"); dal TT2 rifiuta (il torneo è iniziato).

**Implicazioni di design:**
- Serve la creazione del profilo **atomica e idempotente** (unica fonte `createProfileForAccount`): l'auto-join a start e la dichiarazione verificano PRIMA che il profilo non esista (ADR-019).
...
**Contesto e scenario.** Non esiste più alcuna "fase di iscrizione" da aprire (4.1, ADR-009): l'iscrizione piattaforma è sempre disponibile via email. Al commissioner serve una vista di sola lettura sugli account: `platform:list [--json]` elenca email, `registerID`, status (`active`/`pending_unsubscribe`/`unsubscribed`), i **due flag di partecipazione** (`auto-join`, `notifiche apertura`; ADR-019) e le date (`created_at`, `unsubscribed_at`) scritte dal clock iniettato (RF-P8).

**Criteri di accettazione (⇐ RF-P7, RF-P8):**
- `platform:list` mostra tutti gli account con `registerID`, email, status, flag di partecipazione e date, in ordine deterministico (per `registerID`).
...
- I flag di partecipazione sono leggibili e aggiornabili via CLI (`platform:preferences`); un cambio flag impatta i tornei successivi (snapshot a `tournament:start`, ADR-019).
...
**Contesto e scenario.** Il commissioner può intervenire su **round presenti e passati**: correggere una **contabilizzazione errata** (es. rieseguire la contabilizzazione o ripristinare lo stato di un pick) e **inserire manualmente un pick** (`pick:register`, LLD §7.4). L'inserimento manuale risolve l'email del profilo e **verifica che l'account piattaforma sia `active`** (nessun bypass del gate, RF-P6/ADR-009). **Ogni override richiede una motivazione**: il pick fuori finestra/deadline accetta `--reason <motivo>` **obbligatorio**, registrata nel log audit strutturato (ADR-008). Il pick manuale è ammesso solo sul **round corrente non contabilizzato** (`round_state.status = 'open'`); su un round già `scored` si usa il flusso di correzione CL9. **Nessuna retroattività multi-round**: non si può inserire pick per turni precedenti al round corrente. Nei **round futuri** non esiste ancora uno stato da correggere: l'anticipo verso il futuro si ottiene aprendo in anticipo un round (US9). **Ingresso tardivo (ADR-019):** un giocatore escluso dopo la chiusura del TT 1 può entrare **solo** con `tournament:join --email <email> --reason <motivo>` (override auditato, profilo creato con pool intatto); senza `--reason` il join tardivo è rifiutato. (L'iscrizione/ripristino manuale di giocatori è **rimossa**: `platform:register` è l'unico comando di creazione account e non crea profili, ADR-009.)
...
| CL2 | Mittente non iscritto invia un pick | **Solo log interno, nessuna risposta** (anti-spam, RF-P4): il messaggio è marcato letto e nessun profilo viene creato. Un iscritto **senza profilo**: nel **TT 1** riceve `tournament_join_rejected` reason `not_in_tournament` (guida alla dichiarazione `PARTECIPO`, ADR-019); **dal TT 2** è rifiutato con risposta (il torneo è iniziato) |
...
| CL5 | Pick con formato illeggibile | Respinto (o richiesta di chiarimento) spiegando il motivo; nel TT 1, un iscritto senza profilo **non riceve un profilo** (nessun auto-join al pick, ADR-019: il chiarimento insegna la formula di partecipazione `PARTECIPO`); un mittente non iscritto non riceve alcuna risposta (RF-P4). Il giocatore può riprovare |
...
| CL10 | Un giocatore chiede di iscriversi a torneo già avviato (o dopo la deadline del TT 1) | L'iscrizione alla **piattaforma** è accettata in qualunque momento (RF-P1): il giocatore resta un iscritto senza profilo e potrà partecipare a un torneo futuro (per auto-join a start se il flag è ON, o dichiarando). Per il torneo corrente, un iscritto senza profilo può entrare **solo entro la chiusura del TT 1** (dichiarazione `PARTECIPO`/`tournament:join`, ADR-019); dopo, la dichiarazione è rifiutata e l'unico ingresso è l'override CLI `tournament:join --reason` |
...
| CL16 | Partecipante del TT 1 senza pick valido alla deadline del TT 1 | I profili esistono già a `tournament:start` (auto-join) o per dichiarazione (ADR-019): chi è in gara ma senza pick valido alla deadline è eliminato per pick mancante (`missing_pick`, 5.2, RF-13) |
...
| ADR-009 | Iscrizione a livello di piattaforma con storage separato e auto-join al TT1 | §2, §4.1-§4.5, §4.7, RF-P1…P8, CL2/CL5/CL10/CL16 (decisione 6 **emendata** da ADR-019) |
| ADR-019 | Partecipazione opt-in (registration ≠ join) e rimozione dell'auto-join al primo pick | §2, §4.1-§4.3, §4.7, RF-P5/RF-P6/RF-10/RF-06, CL2/CL5/CL10/CL16 |
...
|----------|------|-----------|
| 0.6.2 | 2026-08-30 | **Partecipazione opt-in — registration ≠ join (ADR-019)**. RIMOSSO l'auto-join al primo pick (RF-P5, ADR-009 decisione 6): il pick NON crea più profili. Due flag per-account nel DB piattaforma (`tournament_auto_join`, `receive_tournament_start_notification`, default ON, gestiti SOLO via CLI `platform:register`/`platform:preferences`); profilo creato per **auto-join a `tournament:start`** (snapshot unico; late registrant → deve dichiarare) o per **dichiarazione esplicita** (email `PARTECIPO` / CLI `tournament:join`), ammessa nella finestra del TT 1; dopo la chiusura del TT 1 → rifiuto e unico ingresso = override CLI `tournament:join --reason`. Matrice notifiche emendata (RF-P6/RF-06): `tournament_open` ai soli `receive_tournament_start_notification=ON`; `pick_instructions` ai soli partecipanti (cade l'eccezione TT1 per gli iscritti senza profilo); nuove mail `tournament_join_confirmed`/`tournament_already_joined`/`tournament_join_rejected`; CTA `PARTECIPO` nella mail di apertura torneo; nuovo intento `join` (formula `PARTECIPO`) nel classificatore e nel parser deterministico. RF-10 senza auto-join; US1/US7/US10 riscritte; CL2/CL5/CL10/CL16 riscritti; glossario (registration/join, flag) e tracciabilità §11 aggiornati |
