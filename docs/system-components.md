...
flowchart TB
    ...
    subgraph INFRA["INFRASTRUTTURA"]
        cfg["Config (zod)<br/>parseConfig() · getConfig()<br/>parametri gioco, infra, API, scheduler,<br/>DB_PATH + PLATFORM_DB_PATH (ADR-009)<br/>WIN_ONLY (ADR-016) · AUTOPICK_ON_MISSING (ADR-017)<br/>JOLLIES_PER_PLAYER (ADR-018)"]
        logger["Logger (pino)<br/>createLogger()"]
        sqlite["SQLite (DB torneo)<br/>createConnection() · migrate()<br/>tabelle: player · profile · pick · team<br/>match · round_state · tournament_state<br/>(tournament_state.win_only, ADR-016<br/>autopick_on_missing + pick.auto_pick, ADR-017<br/>jollies_per_player · profile.jollies_remaining<br/>pick.jolly_used, ADR-018)"]
    end

    subgraph PLATFORM["PIATTAFORMA (src/platform) — ADR-009, storage separato"]
        platIface["«PlatformRegistry» (interfaccia)<br/>register() · unregister()<br/>beginUnsubscribe() · confirmUnsubscribe()<br/>reactivate() · find() · activeEmails() · list()<br/>activeAccounts() · setPreferences() (ADR-019)"]
        dbPlat["DbPlatformRegistry<br/>account: registerID stabile · status<br/>active / pending_unsubscribe / unsubscribed<br/>flag opt-in: receiveTournamentStartNotification<br/>tournamentAutoJoin (ADR-019)<br/>created_at dal clock iniettato (RF-P8)"]
        platSqlite["SQLite (DB piattaforma)<br/>PLATFORM_DB_PATH<br/>migratePlatform() · platform_account<br/>+ 2 colonne flag ADR-019 (default 1)"]
    end
    ...
    subgraph GAME["GAME ENGINE (src/game) — nucleo deterministico"]
        ...
        pickProc["Pick Processor<br/>validatePick() — cascata (incl. jolly gates<br/>jolly_not_allowed/no_jollies_left, ADR-018)<br/>checkAcceptance() (guard RF-31)<br/>insertPendingPick() · registerPick()<br/>(decremento atomico jollies_remaining)<br/>listPicks()"]
        reg["Registration (ADR-019)<br/>createProfileForAccount() — unica fonte profilo<br/>autoJoinProfilesAtStart() (auto-join a start)<br/>declareParticipation() (join, DeclareResult)<br/>RIMOSSO autoJoinFromPick (RF-P5)<br/>inizializza jollies_remaining (ADR-018)"]
        ...
        roundMgr["Round Manager<br/>openRound() (RF-14) · closeRound()<br/>scoreRound() (ADR-003) · riepilogo<br/>round_closed_survived (RF-P6, summary_sent)<br/>auto-assign a chiusura (ADR-017: WIN_ONLY &&<br/>AUTOPICK_ON_MISSING e deadline !== null)<br/>salvataggio dal pareggio via jolly (ADR-018)<br/>roundStatus() · roundDeadline()"]
        tourn["Tournament<br/>startTournament() (RF-20/21 + auto-join bulk<br/>autoJoinProfilesAtStart e broadcast tournament_open<br/>filtrato su receiveTournamentStartNotification, ADR-019)<br/>status · history · leaderboard · export"]
        sim["Simulation<br/>simulateSeason() · simulateRound()<br/>mulberry32(seed) (RNF1)<br/>account piattaforma registrati PRIMA di start<br/>+ auto-join a tournament:start (ADR-019)"]
        sched["Scheduler<br/>computeActions() · schedulerTick()<br/>schedulerStatus() (R5–R7)<br/>senza azioni finestra iscrizione (ADR-009)"]
        mode["Mode Guard / GameMode<br/>assertModeConsistent() (ADR-016/017/018)<br/>fatal su cambio WIN_ONLY, AUTOPICK_ON_MISSING<br/>o JOLLIES_PER_PLAYER a torneo aperto<br/>GameMode {winOnly, jollyEnabled} + modeFor()"]
    end
    ...
    subgraph LLM["LLM ADAPTER (src/llm) — confine I/O (ADR-004)"]
        ...
        llmClass["«LLMIntentClassifier» (interfaccia)<br/>classify() → {intent, pick}<br/>subscribe / unsubscribe / join / pick / other<br/>(join = PARTECIPO, ADR-019)"]
        ...
        templates["Templates<br/>EMAIL_TEMPLATES (21 tipi, ADR-009/017/019)<br/>pick_auto_assigned (ADR-017)<br/>tournament_join_confirmed / tournament_already_joined<br/>tournament_join_rejected (ADR-019)<br/>serializeEmailContext()"]
    ...
    subgraph CH["CHANNEL ADAPTER — EMAIL (src/channel)"]
        ...
        emailProc["Email Processor<br/>processEmailBatch()<br/>currentOpenRound() (D8)<br/>subscribe/unsubscribe a due passi (RF-P2)<br/>ramo join → declareParticipation (ADR-019)"]
    end
    ...
    tourn -->|"activeAccounts() → auto-join + broadcast (RF-P6/ADR-019)"| platIface
    ...
    sim -->|"autoJoinProfilesAtStart() (ADR-019)"| reg
    ...
    emailProc -->|"declareParticipation() (join, ADR-019)"| reg
...
- **Piattaforma separata (ADR-009):** `PlatformRegistry` vive su `PLATFORM_DB_PATH` (connessione distinta dal DB torneo) ed è **SOLA LETTA** dai flussi di torneo (gate eligibilità/notifiche/pick): nessuna scrittura cross-DB. La **partecipazione è opt-in (ADR-019)**: i profili nascono per **auto-join a `tournament:start`** (`autoJoinProfilesAtStart`) o per **dichiarazione esplicita** (`declareParticipation`, `PARTECIPO`/`tournament:join`); i due flag per-account (`receiveTournamentStartNotification`, `tournamentAutoJoin`) vivono qui e sono gestiti SOLO via CLI. RIMOSSO `autoJoinFromPick` (RF-P5).
...
| **Config** | `src/config.ts` | Parametri di sistema validati con zod (LLD §4), incl. `PLATFORM_DB_PATH`, `WIN_ONLY` (ADR-016), `AUTOPICK_ON_MISSING` (ADR-017, default `false`, attiva solo con `WIN_ONLY=true`) e `JOLLIES_PER_PLAYER` (ADR-018, int ≥ 0, default `1`, `0` = feature off) | `parseConfig(env)`, `getConfig()`, `ConfigError` | Usata da CLI e logger; mai dai moduli di gioco (via contesto) |
...
| **SQLite DB piattaforma** | `src/db/platform-schema.ts` | Storage SEPARATO degli account (ADR-009, RF-P7): `platform_account` (registerID, email, status, created_at, unsubscribed_at + **2 colonne flag ADR-019** `receive_tournament_start_notification`/`tournament_auto_join`, `INTEGER NOT NULL DEFAULT 1`, in DDL e migrazione additiva) | `migratePlatform(db)`, `applyPlatformAdditiveMigrations()`, `PLATFORM_SCHEMA_DDL` | Usato da DbPlatformRegistry; mai dal DB torneo |
| **PlatformRegistry** | `src/platform/registry.ts` | Archivio account della piattaforma, SOLO LETTO dai flussi di torneo; flag opt-in ADR-019 | `register()` (scrive i due flag a true alla PRIMA creazione), `beginUnsubscribe()`, `confirmUnsubscribe()`, `reactivate()`, `unregister()`, `find()`, `activeEmails()`, **`activeAccounts()`** (account active con flag, ordinati per register_id), **`setPreferences(email, prefs)`** (scrive i flag senza timestamp), `list()`; impl `DbPlatformRegistry` (clock iniettato, RF-P8) | Usato da Eligibility, Email Processor, Tournament (auto-join + broadcast), Simulation, CLI `platform:*` |
...
| **Pick Processor** | `src/game/pick-processor.ts` | Validazione (cascata a 8 passi) e registrazione pick; guard anti-frode RF-31; `insertPendingPick(..., autoPick, jollyUsed)` scrive `pick.auto_pick` (ADR-017) e `pick.jolly_used` (ADR-018); jolly gates `jolly_not_allowed`/`no_jollies_left` dopo `pick_already_exists`; decremento atomico di `profile.jollies_remaining` in `registerPick` | `validatePick()`, `checkAcceptance()` (min(deadline, kickoff effettivo)), `insertPendingPick()`, `registerPick()`, `listPicks()` | Usa Rules, Round Time, provider; usato da CLI, Email Processor, Registration (auto-join), Simulation |
| **Registration** | `src/game/registration.ts` | Partecipazione opt-in (ADR-019) — creazione profilo e DUE ingressi al torneo (auto-join a `tournament:start` + dichiarazione esplicita); inizializza `jollies_remaining` a `JOLLIES_PER_PLAYER` (ADR-018); RIMOSSI `autoJoinFromPick` (RF-P5), `registerPlayer`/`openRegistration`/`closeRegistration`/`autoRegisterFromPick` (ADR-009) | `createProfileForAccount()` (UNICA fonte profilo: riuso/backfill player legacy + `jollies_remaining`), `autoJoinProfilesAtStart()` (snapshot a start, idempotente, no-op senza registry), `declareParticipation()` (join con esito `DeclareResult`: ok / not_active / no_tournament / already_joined / late_requires_reason) | Usa Eligibility, Turn; usato da Tournament, Email Processor, Simulation |
...
| **Round Manager** | `src/game/round-manager.ts` | Ciclo di vita dei round (unico scrittore di `round_state`); notifiche filtrate su account `active` (RF-P6); riepilogo `round_closed_survived` ai soli sopravvissuti alla transizione `closed→scored` (guardia `summary_sent`); **auto-assign** alla chiusura (ADR-017: con `WIN_ONLY && AUTOPICK_ON_MISSING` e `round_state.deadline !== null` assegna ai mancanti la prima squadra disponibile per short_name, email `pick_auto_assigned`, `auto_pick=1`; con deadline NULL elimina `missing_pick` come oggi); **salvataggio dal pareggio via jolly** (ADR-018: in `evaluatePick`, pick con `jolly_used=1` e pareggio → `correct` senza eliminazione, flag runtime `savedByJolly`; righe giocatore con `jolly: true`) | `openRound()` (deadline fissa RF-14), `closeRound()` (esito `RoundCloseResult.autoAssigned`), `scoreRound()` (incrementale ADR-003, idempotente RF-17, Freeze CL1/CL8), `roundStatus()`, `roundDeadline()` | Usa Elimination, Rules, Round Time, Turn, ChannelAdapter, LLMGenerator, PlatformRegistry; usato da CLI, Scheduler, Simulation |
| **Tournament** | `src/game/tournament.ts` | Avvio stagione (RF-20/21) + **auto-join bulk** e broadcast `tournament_open` (RF-P6/ADR-019) e viste aggregate | `startTournament()` (seam `allowPastDeadline`; invoca `autoJoinProfilesAtStart` → campo `autoJoined`; broadcast ai soli account `active` con `receiveTournamentStartNotification`), `tournamentStatus()` (`platformSubscribers`), `tournamentHistory()`, `tournamentLeaderboard()`, `tournamentExport()` | Usa Winner, Rules, Turn, provider, PlatformRegistry, Registration (auto-join); usato da CLI e Simulation |
| **Simulation** | `src/game/simulation.ts` | Riproduzione full-season/round su dati storici (CS3, RNF1) con account piattaforma + auto-join a `tournament:start` | `simulateSeason()` (registra gli account PRIMA di `startTournament`), `simulateRound()` (crea `tournament_state` a mano + invoca esplicitamente `autoJoinProfilesAtStart`), `mulberry32(seed)`, guardie (R3, DB piattaforma pulito, registry obbligatorio) | Invoca Tournament, Registration (auto-join), Pick Processor, Rules, Round Manager, Round Time, Winner, PlatformRegistry; usato da `simulate:*` |
...
| **Mode Guard / GameMode** | `src/game/mode.ts` | Guardia di consistenza della modalità di gioco (ADR-016 win_only, ADR-017 autopick, ADR-018 jolly): rileva un cambio di `WIN_ONLY`/`AUTOPICK_ON_MISSING`/`JOLLIES_PER_PLAYER` a torneo APERTO e abbatte il processo PRIMA di ogni scrittura/invio (throw fatale). Definisce anche l'oggetto **`GameMode { winOnly, jollyEnabled }`** con la factory pura **`modeFor(winOnly, jolliesPerPlayer)`** (unico punto di derivazione della modalità, iniettato a renderer/generatori/template e al parser) | `assertModeConsistent(ctx)` — nome GENERICO (estendibile per chiave a futuri parametri di modalità); `modeFor()` | Usa `getTournamentState()` (Tournament); invocata all'inizio da Scheduler, Round Manager, Pick Processor, Email Processor |
...
| **LLMIntentClassifier** | `src/llm/intent-classifier.ts` | Intento + pick in UNA chiamata LLM (ADR-009, RF-P1/P2); intento `join` (formula `PARTECIPO`, ADR-019); campo `jolly` nel pick e prompt con keyword "jolly" quando `jollyEnabled` (ADR-018) | `classify(body, opts)` → `{intent, pick}`; intenti `subscribe`/`unsubscribe`/`join`/`pick`/`other`; `other`/`pick:null` su contenuto (CS7), `LLMError` su trasporto; filtro esatto (D2/C); impl `OpenAIIntentClassifier` | Usa OpenAIClient e aliases; usato da Email Processor e `llm:classify` |
| **DeterministicIntentClassifier** | `src/llm/deterministic-parser.ts` | Classificatore deterministico dell'input email (ADR-014/019): formule univoche `ISCRIZIONE [NOME]`/`DISISCRIZIONE`/`PARTECIPO`/`<TEAM> <ESITO>`; win_only-aware (ADR-016: squadra nuda → `win`); jolly-aware (ADR-018: keyword "jolly" ovunque con `jollyEnabled` → `jolly: true`, rimossa prima della risoluzione) | `classify()`; `FallbackIntentClassifier` su `LLMError` | Usato da Email Processor e `llm:parse/classify --mode deterministic` |
| **LLMGenerator** | `src/llm/generator.ts` | Testi email in italiano per ogni `EmailType` (21 tipi, ADR-009/017/019) | `generate(ctx)`; `subjectFor()` (D1); `SUBJECT_LABELS.pick_auto_assigned = 'Pick Auto Assegnato'` (ADR-017), `SUBJECT_LABELS.tournament_join_confirmed = 'Partecipazione Confermata'` / `'Già in Gara'` / `'Partecipazione Non Confermata'` (ADR-019); impl `OpenAIGenerator` (placeholder TT/TC, D4/RF-25) | Usa OpenAIClient e Templates; usato da Round Manager, Email Processor, Tournament |
...
| **Templates** | `src/llm/templates.ts` | Template di sistema per tipo email | `EMAIL_TEMPLATES` (21 tipi, incl. `platform_registered`, `platform_unsubscribe_confirm`, `platform_unsubscribed`, `tournament_open`, `round_closed_survived`, `pick_auto_assigned` — ADR-017, e i 3 tipi di partecipazione `tournament_join_confirmed`/`tournament_already_joined`/`tournament_join_rejected` — ADR-019), overlay jolly `JOLLY_TEMPLATE_OVERRIDES`/`narrativeFor(type, mode, savedByJolly)` (ADR-018), `DETERMINISTIC_NARRATIVES` (incl. CTA `PARTECIPO` in `tournament_open`), `serializeEmailContext()`, `formatItDate()` | Usato da Generator |
...
| **Email Processor** | `src/channel/email-processor.ts` | Wiring end-to-end delle email in ingresso (nessuna logica di gioco) | `processEmailBatch()` (router → classificatore → subscribe/unsubscribe a due passi / **ramo join → `declareParticipation` (ADR-019)** / pick SENZA auto-join / silenzio anti-spam → risposte → `\Seen`; errore LLM = stop batch; mittenti rivalutati per messaggio; jolly: `jollyEnabled` nel classify, `jolly` a registerPick, conferma "CON JOLLY" + `jolliesRemaining`, motivi jolly in italiano), `currentOpenRound()` (D8) | Usa Router, Intent Classifier, PlatformRegistry, Pick Processor, Registration, Turn, Generator, ChannelAdapter |
